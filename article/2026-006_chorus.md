...
This step is worth pausing on, because it is the one place where the choice of oracle is not optional. As noted above, rs12740374 sits between CELSR2 and PSRC1, and the SORT1 promoter is on the order of 100 kb away (≈118 kb to the nearest SORT1 TSS in hg38). That distance is far outside the windows of the short-range oracles: ChromBPNet sees about 2 kb and LegNet 200 bp, so neither can connect this variant to SORT1 at all, and even Enformer's roughly 115 kb output window is marginal here (118 kb is just past its 114.7 kb output window). Only a long-context model can span both the variant and the gene, so the gene-level readout comes from AlphaGenome, whose 1 Mb window comfortably covers SORT1. This is exactly why, in the multi-oracle readout in Figure 3, ChromBPNet and LegNet read "not scored" for the gene layer: not because they failed, but because the question is outside what they were built to see.
