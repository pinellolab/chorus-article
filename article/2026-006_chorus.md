...
> ChromBPNet's full 2,114 bp input window centered on the variant (a fresh re-run gives +1.37).
> Reproduces on the conversational path as of chorus **main ≥ commit `9f21266`** (PR #93, which
> fixed a window-collapse bug that previously made the conversational path return ≈ +0.32 for
> fixed-input oracles; AlphaGenome was always unaffected). On older chorus, use an explicit
> input-width window, e.g.
> `oracle.predict_variant_effect("chr1:109273911-109276025", "chr1:109274968", ["G","T"], None)`
> with ChromBPNet (DNASE, HepG2). The *direction* (alt opens chromatin) reproduces on every path.
